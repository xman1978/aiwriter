...
public class FixJson {
    ...
    private static final String FIX_JSON_PROMPT = "你是一个JSON格式修复专家。你的任务是根据 fastjson 的报错信息，修复JSON字符串。\n输出要求：1. 仅输出修复后的 JSON；2. 不能输出任何解释说明。\n报错信息：\n%s\nJSON字符串：\n%s";
    ...
    public String fixJson(String jsonString, boolean useThink, int maxToken, boolean isExchange, JSONObject extParams, OutputStream outputStream) {
        ...
        try {
            ...
            user.put("role", "user");
            user.put("content", String.format(FIX_JSON_PROMPT, e.getMessage(), jsonString));
            prompt.add(user);
